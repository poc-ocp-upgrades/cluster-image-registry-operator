...
func generateSecurityContext(coreClient coreset.CoreV1Interface, namespace string) (*corev1.PodSecurityContext, error) {
	...
	return &corev1.PodSecurityContext{FSGroup: &gid}, nil
}
...
func makePodTemplateSpec(coreClient coreset.CoreV1Interface, driver storage.Driver, params *parameters.Globals, cr *v1.Config) (corev1.PodTemplateSpec, *dependencies, error) {
	...
	env = append(env, corev1.EnvVar{Name: "REGISTRY_HTTP_ADDR", Value: fmt.Sprintf(":%d", params.Container.Port)}, corev1.EnvVar{Name: "REGISTRY_HTTP_NET", Value: "tcp"}, corev1.EnvVar{Name: "REGISTRY_HTTP_SECRET", Value: cr.Spec.HTTPSecret}, corev1.EnvVar{Name: "REGISTRY_LOG_LEVEL", Value: generateLogLevel(cr)}, corev1.EnvVar{Name: "REGISTRY_OPENSHIFT_QUOTA_ENABLED", Value: "true"}, corev1.EnvVar{Name: "REGISTRY_STORAGE_CACHE_BLOBDESCRIPTOR", Value: "inmemory"}, corev1.EnvVar{Name: "REGISTRY_STORAGE_DELETE_ENABLED", Value: "true"}, corev1.EnvVar{Name: "REGISTRY_OPENSHIFT_METRICS_ENABLED", Value: "true"}, corev1.EnvVar{Name: "REGISTRY_OPENSHIFT_SERVER_ADDR", Value: fmt.Sprintf("%s.%s.svc:%d", params.Service.Name, params.Deployment.Namespace, params.Container.Port)})
	if cr.Spec.ReadOnly {
	...
	if cr.Spec.Requests.Read.MaxRunning != 0 || cr.Spec.Requests.Read.MaxInQueue != 0 {
		...
		env = append(env, corev1.EnvVar{Name: "REGISTRY_OPENSHIFT_REQUESTS_READ_MAXRUNNING", Value: fmt.Sprintf("%d", cr.Spec.Requests.Read.MaxRunning)}, corev1.EnvVar{Name: "REGISTRY_OPENSHIFT_REQUESTS_READ_MAXINQUEUE", Value: fmt.Sprintf("%d", cr.Spec.Requests.Read.MaxInQueue)}, corev1.EnvVar{Name: "REGISTRY_OPENSHIFT_REQUESTS_READ_MAXWAITINQUEUE", Value: fmt.Sprintf("%s", cr.Spec.Requests.Read.MaxWaitInQueue)})
	}
	if cr.Spec.Requests.Write.MaxRunning != 0 || cr.Spec.Requests.Write.MaxInQueue != 0 {
		...
		env = append(env, corev1.EnvVar{Name: "REGISTRY_OPENSHIFT_REQUESTS_WRITE_MAXRUNNING", Value: fmt.Sprintf("%d", cr.Spec.Requests.Write.MaxRunning)}, corev1.EnvVar{Name: "REGISTRY_OPENSHIFT_REQUESTS_WRITE_MAXINQUEUE", Value: fmt.Sprintf("%d", cr.Spec.Requests.Write.MaxInQueue)}, corev1.EnvVar{Name: "REGISTRY_OPENSHIFT_REQUESTS_WRITE_MAXWAITINQUEUE", Value: fmt.Sprintf("%s", cr.Spec.Requests.Write.MaxWaitInQueue)})
	}
	...
	vol := corev1.Volume{Name: "registry-tls", VolumeSource: corev1.VolumeSource{Projected: &corev1.ProjectedVolumeSource{Sources: []corev1.VolumeProjection{{Secret: &corev1.SecretProjection{LocalObjectReference: corev1.LocalObjectReference{Name: v1.ImageRegistryName + "-tls"}}}}}}}
	...
	env = append(env, corev1.EnvVar{Name: "REGISTRY_HTTP_TLS_CERTIFICATE", Value: "/etc/secrets/tls.crt"}, corev1.EnvVar{Name: "REGISTRY_HTTP_TLS_KEY", Value: "/etc/secrets/tls.key"})
	vol = corev1.Volume{Name: "registry-certificates", VolumeSource: corev1.VolumeSource{ConfigMap: &corev1.ConfigMapVolumeSource{LocalObjectReference: corev1.LocalObjectReference{Name: v1.ImageRegistryCertificatesName}}}}
	volumes = append(volumes, vol)
	...
	resources := corev1.ResourceRequirements{Requests: corev1.ResourceList{corev1.ResourceCPU: resource.MustParse("100m"), corev1.ResourceMemory: resource.MustParse("256Mi")}}
	if cr.Spec.Resources != nil {
		resources = *cr.Spec.Resources
	}
	spec := corev1.PodTemplateSpec{ObjectMeta: metav1.ObjectMeta{Labels: params.Deployment.Labels}, Spec: corev1.PodSpec{Tolerations: cr.Spec.Tolerations, NodeSelector: cr.Spec.NodeSelector, PriorityClassName: "system-cluster-critical", Containers: []corev1.Container{{Name: "registry", Image: image, Ports: []corev1.ContainerPort{{ContainerPort: int32(params.Container.Port), Protocol: "TCP"}}, Env: env, VolumeMounts: mounts, LivenessProbe: generateLivenessProbeConfig(params), ReadinessProbe: generateReadinessProbeConfig(params), Resources: resources}}, Volumes: volumes, ServiceAccountName: params.Pod.ServiceAccount, SecurityContext: securityContext, Affinity: &corev1.Affinity{PodAntiAffinity: &corev1.PodAntiAffinity{PreferredDuringSchedulingIgnoredDuringExecution: []corev1.WeightedPodAffinityTerm{{Weight: 100, PodAffinityTerm: corev1.PodAffinityTerm{TopologyKey: "kubernetes.io/hostname", Namespaces: []string{params.Deployment.Namespace}}}}}}}}
