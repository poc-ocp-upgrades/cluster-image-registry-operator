...
func (s *StatusHandler) Create() error {
	...
	state := &configapiv1.ClusterOperator{ObjectMeta: metaapi.ObjectMeta{Name: s.Name}, Status: configapiv1.ClusterOperatorStatus{}}
...
func (s *StatusHandler) Update(condtype configapiv1.ClusterStatusConditionType, condstate ConditionState, newVersion string) error {
	...
	return retry.RetryOnConflict(retry.DefaultBackoff, func() error {
		...
		if err != nil {
			...
			modified = true
			state = &configapiv1.ClusterOperator{ObjectMeta: metaapi.ObjectMeta{Name: s.Name}, Status: configapiv1.ClusterOperatorStatus{Conditions: []configapiv1.ClusterOperatorStatusCondition{{Type: configapiv1.OperatorAvailable, Status: configapiv1.ConditionUnknown, LastTransitionTime: metaapi.Now()}, {Type: configapiv1.OperatorProgressing, Status: configapiv1.ConditionUnknown, LastTransitionTime: metaapi.Now()}, {Type: configapiv1.OperatorFailing, Status: configapiv1.ConditionUnknown, LastTransitionTime: metaapi.Now()}}}}
			sdkFunc = client.ClusterOperators().Create
		}
		modified = updateOperatorCondition(state, &configapiv1.ClusterOperatorStatusCondition{Type: condtype, Status: condstate.Status, Message: condstate.Message, Reason: condstate.Reason, LastTransitionTime: metaapi.Now()})
		if len(newVersion) > 0 {
			newVersions := []configapiv1.OperandVersion{{Name: "operator", Version: newVersion}}
