...
func createPV(t *testing.T, storageClass string) error {
	...
	for i := 0; i < 100; i++ {
		...
		pv := &corev1.PersistentVolume{ObjectMeta: metav1.ObjectMeta{Name: pvName}, Spec: corev1.PersistentVolumeSpec{Capacity: corev1.ResourceList{corev1.ResourceStorage: resource.MustParse("100Gi")}, AccessModes: []corev1.PersistentVolumeAccessMode{corev1.ReadWriteOnce, corev1.ReadOnlyMany, corev1.ReadWriteMany}, PersistentVolumeSource: corev1.PersistentVolumeSource{HostPath: &corev1.HostPathVolumeSource{Path: localPath}}, StorageClassName: storageClass}}
...
func TestDefaultPVC(t *testing.T) {
	...
	framework.MustDeployImageRegistry(t, client, &imageregistryv1.Config{TypeMeta: metav1.TypeMeta{APIVersion: imageregistryv1.SchemeGroupVersion.String(), Kind: "Config"}, ObjectMeta: metav1.ObjectMeta{Name: imageregistryv1.ImageRegistryResourceName}, Spec: imageregistryv1.ImageRegistrySpec{ManagementState: operatorapi.Managed, Storage: imageregistryv1.ImageRegistryConfigStorage{PVC: &imageregistryv1.ImageRegistryConfigStoragePVC{}}, Replicas: 1}})
